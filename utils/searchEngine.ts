...
import { tokenize, stem, DOMAIN_SYNONYMS, BERKELEY_VENUE_ALIASES } from './textUtils';
...
const RE_AFTERNOON = /\b(afternoon|lunch|midday|after class|after lunch|noon)\b/i;
...
const CATEGORY_PATTERNS: Array<[string, RegExp]> = [
  ['Entrepreneurship', /\b(startup|founder|venture|pitch|demo day|skydeck|entrepreneur|entrepreneurship|product management|innovation hub)\b/i],
  ['Sports',           /\b(cal game|bears game|cal bears|athletics|basketball|football|baseball|volleyball|soccer|swim meet|swim team|tennis|gymnastics|rowing|crew|sports)\b/i],
  ['Arts',             /\b(film screening|film|movie|concert|performance|theater|theatre|gallery|bampfa|dance|opera|recital|exhibition|exhibit|museum)\b/i],
  ['Science & Tech',   /\b(ai\b|machine learning|data science|hackathon|coding|computer science|eecs|engineering talk|robotics|biotech|genomics|tech talk)\b/i],
  ['Student Life',     /\b(free food|student org|club|social|mixer|orientation|undergrad|grad student|tabling|info session|open house|coffee chat)\b/i],
  ['Academic',         /\b(seminar|colloquium|lecture|symposium|dissertation defense|dissertation|thesis defense|guest speaker|research talk|keynote)\b/i],
];

// Campus area patterns
const AREA_PATTERNS: Array<[SearchFilter['campusArea'], RegExp, string]> = [
  ['northside', /\b(northside|north side|northgate|euclid|hearst|north campus)\b/i, 'Northside'],
  ['southside', /\b(southside|south side|telegraph|south campus)\b/i,               'Southside'],
  ['downtown',  /\b(downtown berkeley|shattuck|bart)\b/i,                           'Downtown'],
];

// Known multi-word phrases to detect and boost
const KNOWN_PHRASES = [
  'free food', 'film screening', 'career fair', 'startup founder', 'guest speaker',
  'study group', 'coffee chat', 'info session', 'open house', 'demo day',
  'tech talk', 'research talk', 'panel discussion', 'happy hour', 'game night',
  'networking event', 'dissertation defense', 'job fair', 'book club',
  'startup pitch', 'venture capital', 'data science', 'machine learning',
];
...
export function buildSearchPlan(query: string): SearchPlan {
  ...
  if (!raw) {
    return { raw, cleaned, keywords: [], expandedTokens: [], phrases, filters, interpretations };
  }
  ...
  // ── Keywords ──────────────────────────────────────────────────────────────
  const keywords = tokenize(cleaned || raw);
...
function scoreEvent(
  ...
  for (const phrase of plan.phrases) {
    ...
    if (text.includes(phrase)) { score += W.phraseMatch; matched++; }
  }
  ...
  for (const qt of plan.expandedTokens) {
    ...
    if (index.t[qt]?.includes(pos)) { score += W.title    * mult; matched++; }
    if (index.g[qt]?.includes(pos)) { score += W.tag      * mult; matched++; }
    if (index.o[qt]?.includes(pos)) { score += W.org      * mult; matched++; }
    if (index.l[qt]?.includes(pos)) { score += W.location * mult; matched++; }
    if (index.d[qt]?.includes(pos)) { score += W.desc     * mult; matched++; }
  }
...
function applyPoolFilters(events: CalEvent[], plan: SearchPlan, dismissedKeys: Set<string>): CalEvent[] {
  const { filters } = plan;
  return events.filter(ev => {
    // Time-of-day: soft hard filter — only when explicitly detected
    if (filters.timeOfDay && !dismissedKeys.has(`timeOfDay:${filters.timeOfDay}`) && ev.time) {
    ...
    // Modality
    if (filters.modality && !dismissedKeys.has(`modality:${filters.modality}`)) {
      const text = `${ev.title} ${ev.location} ${ev.description ?? ''}`.toLowerCase();
      if (filters.modality === 'online'    && !/\b(online|virtual|zoom|remote|webinar)\b/.test(text)) return false;
      if (filters.modality === 'in-person' && /\b(online|virtual|zoom|remote|webinar)\b/.test(text))  return false;
    }
    return true;
  });
}
...
function runScoring(pool: CalEvent[], plan: SearchPlan, index: SearchIndex | null): CalEvent[] {
  ...
  if (index && plan.expandedTokens.length > 0) {
    ...
    for (const pos of candidatePos) {
      ...
      if (score > 0) { scored.push({ event: ev, score }); scoredIds.add(ev.id); }
    }
  }

  // Phase 2: Fuse.js fallback for tokens with zero index hits
  const tokensWithHits = index
    ? new Set(plan.expandedTokens.filter(t =>
        (index.t[t]?.length ?? 0) > 0 || (index.g[t]?.length ?? 0) > 0 ||
        (index.o[t]?.length ?? 0) > 0 || (index.l[t]?.length ?? 0) > 0 ||
  ...
  if (fuzzyTokens.length > 0 || scored.length === 0) {
    const fuzzyPool = scored.length === 0 ? pool : pool.filter(e => !scoredIds.has(e.id));
    ...
    for (const { item, score: fs } of fuse.search(fuseQuery, { limit: 100 })) {
      ...
      else { scored.push({ event: item, score: relevance }); }
    }
  }
  ...
  if (results.length < 3) {
    ...
    if (plan.filters.dateRange && plan.filters.dateRange !== 'upcoming') {
      const relaxedPlan: SearchPlan = {
        ...plan,
        filters: { ...plan.filters, dateRange: plan.filters.dateRange === 'today' ? 'week' : 'upcoming' },
      };
