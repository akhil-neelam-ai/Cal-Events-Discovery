...
function stem(word) {
  ...
  if (!w.endsWith('ss') && !w.endsWith('us') && w.endsWith('s') && w.length > 4) w = w.slice(0, -1);
  if (w.length <= 3) return w;
  if (w.endsWith('ing') && w.length > 6) {
    const base = w.slice(0, -3);
    if (base.length >= 3) return /([bcdfghjklmnpqrstvwxyz])\1$/.test(base) && base.length >= 4 ? base.slice(0, -1) : base;
  }
...
const STOP = new Set(['a','an','the','and','or','in','on','at','to','for','of','with','by','from','is','are','this','that','it']);
...
test('index has location field', () => {
  assert.ok(searchIndex.l && typeof searchIndex.l === 'object', 'l field must exist');
...
test('index covers full description (not just first 150 chars)', () => {
  ...
  assert.ok(anyHit, `expected a tail token from "${longDesc.title}" to be indexed in d`);
});
...
test('search: "ai" returns Science & Tech events', () => {
  ...
  const hasSTEM = hasMatchIn(results, e =>
    e.tags?.some(t => t.toLowerCase().includes('science') || t.toLowerCase().includes('tech'))
  );
  assert.ok(hasSTEM, `Expected Science & Tech in top results for "ai machine learning". Got: ${topTitles('ai machine learning', 5).join(' | ')}`);
});
...
test('search: "film screening" returns Arts events', () => {
  ...
  assert.ok(hasArts, `Expected Arts events for "film screening". Got: ${topTitles('film screening').join(' | ')}`);
});

test('search: "bampfa" returns events from bampfa source', () => {
  const results = indexSearch('bampfa', 10);
  const hasBampfa = hasMatchIn(results, e => e.source === 'bampfa' || e.organizer?.toLowerCase().includes('bampfa'));
  assert.ok(hasBampfa, `Expected bampfa events. Got: ${results.slice(0, 5).map(r => `${r.event?.title} (${r.event?.source})`).join(' | ')}`);
});
...
test('search: "startup founder" returns Entrepreneurship events', () => {
  ...
  assert.ok(hasEntrepreneur, `Expected Entrepreneurship events for "startup founder". Got: ${topTitles('startup founder').join(' | ')}`);
});
...
test('search: "free food" matches events mentioning free in title or description', () => {
  ...
  assert.ok(hasFree || results.length === 0, 'free food events should mention free, or no results');
});

test('search: "seminar" returns Academic events', () => {
  const results = indexSearch('seminar', 10);
  const hasAcademic = hasMatchIn(results, e =>
    e.tags?.some(t => t.toLowerCase().includes('academic') || t.toLowerCase().includes('science'))
  );
  assert.ok(hasAcademic, `Expected Academic events for "seminar". Got: ${topTitles('seminar').join(' | ')}`);
});

test('search: "cal bears" or "sports" returns Sports events', () => {
  const results = indexSearch('cal bears sports athletics', 10);
  const hasSports = hasMatchIn(results, e =>
    e.tags?.some(t => t.toLowerCase().includes('sport')) || e.source === 'calbears'
  );
  assert.ok(hasSports, `Expected Sports events. Got: ${topTitles('cal bears', 5).join(' | ')}`);
});

test('search: "haas" returns Haas / business school events', () => {
  const results = indexSearch('haas', 10);
  const hasHaas = hasMatchIn(results, e =>
    e.source === 'haas' ||
    e.organizer?.toLowerCase().includes('haas') ||
    e.title?.toLowerCase().includes('haas')
  );
  assert.ok(hasHaas, `Expected Haas events. Got: ${results.slice(0,5).map(r=>`${r.event?.title} (${r.event?.source})`).join(' | ')}`);
});
