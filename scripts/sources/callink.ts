...
function categorizeCampusGroups(theme: string, categoryNames: string[]): string[] {
  ...
  for (const cat of categoryNames) {
    ...
    if (/\b(academic|education|lecture|seminar|panel|talk|workshop|research|learning)\b/.test(lower)) {
      cats.add('Academic');
    } else if (/\b(art|music|film|performance|gallery|theater|dance|creative)\b/.test(lower)) {
      cats.add('Arts');
    } else if (/\b(sport|athletic|fitness|recreation|intramural)\b/.test(lower)) {
      cats.add('Sports');
    } else if (/\b(tech|computer|engineer|science|data|ai|stem|hack)\b/.test(lower)) {
      cats.add('Science & Tech');
    } else if (/\b(entrepreneur|startup|innovation|venture|business|pitch)\b/.test(lower)) {
...
export async function fetchCallink(): Promise<FetchResult> {
  ...
  if (!response.ok) {
    throw new Error(`CalLink fetch failed: ${response.status} ${response.statusText}`);
  }
  ...
  console.log(`[callink] API returned ${raw.length} items (odata.count: ${apiTotal})`);
  ...
  for (const item of raw) {
    ...
    try {
      ...
      const description = item.description ? stripHtml(item.description) : title;
      const organizer = item.organizationName ?? '';
      const venue = item.location ?? '';
      const categories = categorizeCampusGroups(item.theme ?? '', item.categoryNames ?? []);
      ...
      invalid++;
      if (invalid <= 5) {
        console.warn(`[callink] failed to parse event: ${err instanceof Error ? err.message : err}`);
      }
    }
  }

  console.log(`[callink] parsed ${events.length}/${rawCount} (past: ${filteredPast}, invalid: ${invalid})`);
