...
export default function App() {
  ...
  const loadDailyBatch = async (force: boolean = false) => {
    setLoading(LoadingState.LOADING);
    try {
      const data = await fetchEventsFromGemini(force);
      ...
      <header className="bg-berkeley-blue text-white shadow-md sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="w-full md:w-1/2 relative">
            ...
            />
            <button 
              onClick={() => loadDailyBatch(true)} 
              title="Force Refresh Sync"
              className="absolute right-2 top-1/2 transform -translate-y-1/2 text-berkeley-blue p-1.5 hover:bg-gray-100 rounded-full transition"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${loading === LoadingState.LOADING ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {loading === LoadingState.LOADING && (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <div className="text-center">
              <h3 className="text-berkeley-blue font-bold text-lg">Daily Snapshot Sync</h3>
              <p className="text-gray-500 text-sm animate-pulse">Scanning campus calendars for today's batch...</p>
            </div>
          </div>
        )}
        ...
        {loading === LoadingState.ERROR && (
          <div className="text-center py-10 bg-red-50 rounded-xl border border-red-200 max-w-lg mx-auto">
            <h3 className="text-xl text-red-800 font-bold mb-2">Sync Failed</h3>
            <p className="text-red-600 mb-4">We couldn't prepare today's batch of events.</p>
            <button onClick={() => loadDailyBatch(true)} className="px-6 py-2 bg-berkeley-blue text-white rounded-lg font-bold hover:bg-berkeley-medblue transition shadow-md">Retry Sync</button>
          </div>
        )}
